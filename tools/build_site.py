...
def build():
    ...
    for file in ROOT.iterdir():
        if file.is_file() and (file.suffix in ('.html', '.ico', '.xml') or file.name in ('CNAME', 'robots.txt')):
    ...
    dates = {n.findtext('s:loc', namespaces=ns): n.findtext('s:lastmod', namespaces=ns) for n in old.getroot()}
    ET.register_namespace('', ns['s'])
    ET.register_namespace('xhtml', 'http://www.w3.org/1999/xhtml')
    sitemap = ET.Element('{' + ns['s'] + '}urlset')
    for url, lastmod in dates.items():
        if urlsplit(url).path.startswith(('/learn/', '/ja/learn/')):
            continue
    ...
    for path in sources:
        for lang in ('en', 'ja'):
            item = ET.SubElement(sitemap, 'url')
            ET.SubElement(item, 'loc').text = ORIGIN + localized(path, lang)
            if dates.get(ORIGIN + path):
                ET.SubElement(item, 'lastmod').text = dates[ORIGIN + path]
            for alternate in ('en', 'ja', 'x-default'):
